...
(async () => {
  try {
    ...
    for (const spreadsheets of config.spreadsheets) {
      ...
      // Read url content
      const [responseCode, data] = await httpsRequestAsync(spreadsheetUrl);
      console.log(
        "Response code:",
        responseCode,
        "Downloaded bytes:",
...
async function processRow(row) {
  ...
  const url = row[0];
  const type = row[2];
  let categories = row[3]
    ? row[3].replace(/, /g, "##").replace(/ \//g, ",").split("##")
    : [];
  ...
  if (fs.existsSync(outputfile)) {
    ...
    if (screenshotDate === "undefined" || !screenshotDate) {
  ...
  if (visitWebsites || !lastCrawlDate) {
    ...
    let data = "";
    [responseCode, data] = await httpsRequestAsync(url);
  ...
  if (address.match(/\.pdf$/i)) {
    ...
    !(
      fs.existsSync(largeScreenshotFile) &&
      fs.statSync(largeScreenshotFile).size > 50
    )
  ) {
    takeScreenshot = true;
    screenshotDate = myDateFormat(new Date());
...
`;
  ...
  if (
    ...
    fs.statSync(largeScreenshotFile).size > 50 &&
    !(
      fs.existsSync(publishedScreenshot) &&
      fs.statSync(publishedScreenshot).size > 50
    )
  ) {
